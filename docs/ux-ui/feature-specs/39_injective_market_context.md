# InjSight AI UX/UI Specification — Injective Market Context

## 1. Feature Overview

**Feature:** Injective Market Context  
**Category:** Price and market context  
...
Add token pricing, volatility context, and estimated value.
...
Once you use Injective Market Context, relevant wallet intelligence will appear here.
...
```ts
export interface InjectiveMarketContextProps {
  className?: string;
}
...
```txt
frontend/components/dashboard/InjectiveMarketContext.tsx
frontend/types/injective_market_context.ts
frontend/data/mock_injective_market_context.ts
```
