# InjSight AI UX/UI Specification — Read-Only Safety Messaging

## 1. Feature Overview

**Feature:** Read-Only Safety Messaging  
**Category:** Trust and safety copy  
**Primary Route / Placement:** `Global UI pattern`  
**Product:** InjSight AI  
**Product Subtitle:** AI Wallet Intelligence for Injective DeFi  

## 2. Purpose

Make it clear InjSight AI is non-custodial and read-only.
...
Once you use Read-Only Safety Messaging, relevant wallet intelligence will appear here.
...
```ts
export interface ReadOnlySafetyMessagingProps {
  className?: string;
}
...
```txt
frontend/components/dashboard/ReadOnlySafetyMessaging.tsx
frontend/types/read_only_safety_messaging.ts
frontend/data/mock_read_only_safety_messaging.ts
```
