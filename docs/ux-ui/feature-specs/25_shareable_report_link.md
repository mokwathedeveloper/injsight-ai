# InjSight AI UX/UI Specification — Shareable Report Link

## 1. Feature Overview

**Feature:** Shareable Report Link  
**Category:** Public report sharing  
**Primary Route / Placement:** `Future workflow`  
**Product:** InjSight AI  
**Product Subtitle:** AI Wallet Intelligence for Injective DeFi  

## 2. Purpose

Create safe read-only links for selected reports.
...
Once you use Shareable Report Link, relevant wallet intelligence will appear here.
...
```ts
export interface ShareableReportLinkProps {
  className?: string;
}
...
```txt
frontend/components/dashboard/ShareableReportLink.tsx
frontend/types/shareable_report_link.ts
frontend/data/mock_shareable_report_link.ts
```
