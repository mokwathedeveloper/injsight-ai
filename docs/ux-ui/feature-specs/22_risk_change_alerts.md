# InjSight AI UX/UI Specification — Risk Change Alerts

## 1. Feature Overview

**Feature:** Risk Change Alerts  
**Category:** Risk monitoring events  
**Primary Route / Placement:** `/dashboard/alerts`  
**Product:** InjSight AI  
**Product Subtitle:** AI Wallet Intelligence for Injective DeFi  

## 2. Purpose

Notify users when wallet risk changes meaningfully.
...
Once you use Risk Change Alerts, relevant wallet intelligence will appear here.
...
```ts
export interface RiskChangeAlertsProps {
  className?: string;
}
...
```txt
frontend/components/wallet/RiskChangeAlerts.tsx
frontend/types/risk_change_alerts.ts
frontend/data/mock_risk_change_alerts.ts
```
